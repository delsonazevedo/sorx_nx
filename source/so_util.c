 ...
 * the relocation types they use. Generic across modules: so_resolve_symbol
 * checks the shim table first, then every other already-loaded module's
 * exports, so multiple interdependent .so's (hidapi -> SDL2 -> openbor) can be
 * so_load()'d in dependency order and resolve against each other automatically.
 */
